...
def run_single_chain_simulation(snapshot, prot, residues, folder):
...
folder = f'traj/{args.name}/CALVADOS/1chain'
...
for irep in range(args.replica):
    with hoomd.context.SimulationContext():
        run_single_chain_simulation(snapshot, prot, residues, f'{folder}/replica_{irep}')