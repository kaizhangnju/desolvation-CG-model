...
e_ratio = Decimal(args.eps_scale) # Scaling factor for epsilon values
eh = (Decimal('0.2') * e_ratio).normalize()
eb = (Decimal('0.2') * Decimal(args.alpha_b) * e_ratio).normalize()
ew = (Decimal('0.2') * Decimal(args.alpha_w) * e_ratio).normalize()

# GENERATE FOLDERS
fileroot = str(production_T)
folder = f'traj/{protein}/CAL_{eb}_{ew}_{e_ratio}/{production_T}'
